Copy (WITH conditions_extract AS (
   SELECT id, 
      ...
      WHERE id IN 
      (SELECT condition FROM actions WHERE (study = 9 OR study = 8))
   ),
   ...
   (SELECT id, 
      ...
      wage::text::float, training, work_limit FROM conditions_extract),
...
-- select other::json->'activity_log'->0 from actions where study = 9 and action = 'finished'
   ...
   WHERE a.action = 'js log on user leaving' AND a.hitid IS NOT NULL AND a.assid IS NOT NULL) 
	 ...
	 survey_deadline, survey_offered_pay, page_load_time, impro_rate
...
) To '/tmp/data_extract11.csv' With CSV HEADER;
